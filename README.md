...
```groovy
plugins {
  id "io.redskap.swagger-brake" version "2.0.0"
}
```
...
buildscript {
  ...
  dependencies {
    classpath "io.redskap:swagger-brake-gradle:2.0.0"
  }
}
...
- `outputFormats` - The format of the report. Defaults to `["HTML"]`. The parameter accepts a list as multiple report formats are configurable. 
Possible values can be found [here](https://github.com/redskap/swagger-brake#reporting).
